...
pub struct LatencyDiscovery {
    pub sequence_number: u32,
    pub truncated_siphash: u32,
}

impl LatencyDiscovery {
    pub fn serialize(&mut self, crypto: &Crypto, buf: &mut [u8]) -> usize {
        buf[0] = PacketIdentifier::LatencyDiscovery as u8;
    ...
    pub fn deserialize(crypto: &Crypto, buf: &[u8]) -> Result<Self, &'static str> {
        ...
        if PacketIdentifier::try_from(buf[0])? != PacketIdentifier::LatencyDiscovery {
